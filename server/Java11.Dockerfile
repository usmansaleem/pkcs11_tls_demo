...
RUN apt-get -y update && apt-get -y install libssl3 libnss3-tools && rm -rf /var/lib/api/lists/*
WORKDIR /opt
VOLUME /certs

ENTRYPOINT ["java", "-Djavax.net.debug=ssl:handshake:verbose", "Server.java"]